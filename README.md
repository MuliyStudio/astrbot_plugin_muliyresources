...
| 📰 | **软件日报** | 每日定时推送最新软件到群聊（图片形式，不再发送 ZIP 文件） |
| 🎮 | **游戏日报** | 每日定时抓取当前游戏源（XDGAME / switch618）当日更新的游戏，取标题/简介/截图，排版成卡通风格 HTML 后渲染为图片推送到群聊 |
| 🎬 | **影视日报** | 每日定时抓取影视站主页「最近更新」的电影/剧集/动漫，取名称/封面/状态/简介，排版成毛玻璃简约风格 HTML 后渲染为图片推送到群聊；配置了 `muliy_cookie` 走教父.com 新站，未配置自动回退 a123tv 旧站（免登录也能出日报） |
...
- **排版与渲染**：先把游戏排版成**卡通风格 HTML**（圆角贴纸卡片、飘带气泡、波点渐变背景、NEW 角标），再用 Playwright（Chromium）把整页渲染为图片**直接发送**（不再发送 ZIP，图片即高清原图）。
- **容错**：若未安装浏览器（`playwright install chromium`）导致渲染失败，自动降级为「文字版」推送（游戏名 + 分类）。

**命令**：`/game_report` 手动触发当日游戏日报（按当前游戏源抓取）。

**配置**：`game_report_enabled`（总开关）、`game_schedule_hour`/`game_schedule_minute`（独立定时，默认 18:00）、`game_group_ids`（独立推送群）、`game_report_max`（每期上限，默认 8；switch618 源抓全今日更新不受此限），浏览器由统一的「浏览器」分组（`browser_channel` / `browser_exe`）指定。
...
每天（按独立的 `movie_schedule_hour`/`movie_schedule_minute` 与 `movie_group_ids`，与软件/游戏日报分开）自动抓取影视站主页「最近更新」列表，取电影 / 剧集 / 动漫，排版成**毛玻璃简约风格 HTML** 后渲染为图片推送到群聊。
...
- **调度**：APScheduler 定时推送软件日报 / 游戏日报（共用调度器与群配置）
- **图片生成**：Pillow 自定义排版（软件日报图片）；Playwright（Chromium）渲染卡通 HTML 为图片（游戏日报）
