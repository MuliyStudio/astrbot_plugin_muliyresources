...
> 🎉 **新版本默认「内置直连」模式（wyy_backend=direct），零部署即可用**：
> 插件内部直接用网易云官方接口解析 + 扫码登录，不再需要单独部署任何服务，
> 也没有 Docker / `127.0.0.1` 填错的坑。**免费歌曲开箱即用**，`wyy_custom_url` 留空即可。
> **扫码登录也已内置**：直接发 `/wyy_login`，用网易云 App 扫码，会员 Cookie 自动写入 `wyy_cookie`，
> 全程不依赖 NeteaseCloudMusicApi——你可以彻底弃用它。
> 本目录下的部署方式**仅作为 custom 后端的兼容保留**（老用户已部署、想继续用才需要）。

> ⚠️ **重要限制**：网易云音频 CDN（`m*.music.126.net`）会封锁云服务器（数据中心）IP，
> 服务器端下载音频可能返回 **403**（与早年公共解析站被封同源，非插件 bug）。
> 此时插件会自动降级为发送「歌曲名片 + 播放链接」，用户点开即可听。
> 想正常发语音，请把插件部署在**家宽 / 本地网络**（非云服务器 IP）。

> 历史说明：早期版本「网易云语音名片」只支持自建后端（`wyy_custom_url`）。
> 公共解析站（wyapi / qzxdp）已在 v1.9.3 移除（被网易云 WAF 对服务器 IP 拦截），
> 所以当时每台要用的设备都得连到一个自建 NeteaseCloudMusicApi 实例。
> 现在内置直连模式已解决该问题，绝大多数用户无需再看本指南。
